...
def signup():
    ...
    if request.method == 'POST':
        ...
        user_check = get_db().execute(
            "select userid from user where userid=?", [userid,]).fetchall()
        if not user_check:
            db.execute(
    ...
    if request.method == "POST":
        ...
        if user_data is not None:
            if check_password_hash(user_data[0], password):
                ...
                login_user(user)
                return redirect('/')
        error_message = "入力されたID、またはパスワードが誤っています。"
...
def edit(id):
    if request.method == "POST":
        ...
        body = request.form.get('body')
        db = get_db()
        db.execute("update memo set title=?, body=? where id=?",
                   [title, body, id])
        db.commit()
        return redirect('/')
    post = get_db().execute(
        "select id,title,body from memo where id=?",
        (id,)
    ).fetchone()
